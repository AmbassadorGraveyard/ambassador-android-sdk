...
public final class RAFOptions {
    ...
    public static void set(@NonNull RAFOptions rafOptions) {
        ...
        if (AmbSingleton.getInstanceContext() != null) {
            AmbSingleton.getInstanceContext().getSharedPreferences("rafOptions", Context.MODE_PRIVATE).edit().putString(campaignId, data).apply();
        }
    }
    ...
    public static RAFOptions get() {
        if (instance == null) {
            String campaignId = new Campaign().getId();
            SharedPreferences prefs = AmbSingleton.getInstanceContext().getSharedPreferences("rafOptions", Context.MODE_PRIVATE);
